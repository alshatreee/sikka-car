      ...
      <div className="container">
        ...
        <div className="mb-8 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            ...
            >
              {/* Label skeleton */}
              <div className="h-5 w-2/3 rounded-lg bg-dark-surface animate-pulse" />

              {/* Value skeleton */}
              <div className="h-8 w-1/2 rounded-lg bg-status-star/20 animate-pulse" />

              {/* Subtext skeleton */}
              <div className="h-4 w-3/4 rounded-lg bg-dark-surface animate-pulse" />
            </div>
          ))}
        </div>

        {/* Tabs skeleton */}
        <div className="mb-6 flex gap-4 border-b border-dark-border">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-10 w-32 rounded-lg bg-dark-card animate-pulse"
            />
          ))}
        </div>

...
يحاكي 10,000 مسار عشوائي حتى انتهاء السوق
يقارن الاحتمال المحسوب بسعر السوق — إذا وُجدت فجوة >= 10% يدخل
...
STATE_FILE = BASE_DIR / "markov_state.json"
...
def http_get(url: str, timeout: int = 15):
    try:
        ...
        log(f"HTTP خطأ: {url[:60]} — {e}", "WARNING")
        return None
...
def fetch_active_markets() -> list[dict]:
    """يجلب أسواق نشطة مع فلترة حسب السعر والحجم والوقت."""
    ...
    for m in markets:
        ...
        try:
            ...
            continue
        if hours_left < MIN_HOURS_LEFT or hours_left > MAX_DAYS_LEFT * 24:
            continue
        # استخراج الأسعار
        ...
        if not (MIN_PRICE <= yes_p <= MAX_PRICE):
            continue
        # استخراج token_ids
...
def fetch_price_history(token_id: str) -> list[float]:
    """يجلب تاريخ أسعار ساعية — CLOB API أو بناء اصطناعي."""
    if token_id:
        data = http_get(
            f"{CLOB_API}/prices-history?market={token_id}"
            f"&interval=1h&fidelity=60")
        if data and isinstance(data, dict):
            hist = data.get("history", [])
            if isinstance(hist, list) and len(hist) >= 24:
                prices = []
                for pt in hist:
                    p = float(pt.get("p", 0) or pt.get("price", 0) or 0)
                    if 0 < p < 1:
                        prices.append(p)
                if len(prices) >= 24:
                    return prices
        ...
        data2 = http_get(
            f"{CLOB_API}/prices-history?market={token_id}"
            f"&interval=max&fidelity={LOOKBACK_DAYS * 24}")
        if data2 and isinstance(data2, dict):
            hist2 = data2.get("history", [])
            if isinstance(hist2, list) and len(hist2) >= 24:
                prices = [float(pt.get("p", 0) or pt.get("price", 0)) for pt in hist2
...
def synthesize_history(current_price: float, hours: int = LOOKBACK_DAYS * 24) -> list[float]:
    """يبني تاريخ اصطناعي من السعر الحالي بتقلب واقعي."""
    rng = random.Random(int(current_price * 10000))
    volatility = 0.008
    prices = [current_price]
    p = current_price
    for _ in range(hours - 1):
        change = rng.gauss(0, volatility)
        p = max(0.01, min(0.99, p + change))
        prices.append(round(p, 4))
    return prices

# ═══════════════════════════════════════════════
# بناء مصفوفة انتقال ماركوف
# ═══════════════════════════════════════════════
def price_to_bucket(price: float) -> int:
    b = int(price * N_BUCKETS)
    return max(0, min(N_BUCKETS - 1, b))

def build_transition_matrix(prices: list[float]):
    """يبني مصفوفة 10×10 من تحولات الأسعار الساعية."""
    if HAS_NUMPY:
    ...
    for i in range(len(prices) - 1):
        frm = price_to_bucket(prices[i])
        to = price_to_bucket(prices[i + 1])
        if HAS_NUMPY:
            matrix[frm, to] += 1.0
        else:
            matrix[frm][to] += 1.0

    # تطبيع كل صف — مع smoothing لابلاس
    for r in range(N_BUCKETS):
        if HAS_NUMPY:
            row_sum = matrix[r].sum()
            if row_sum == 0:
                matrix[r] = np.ones(N_BUCKETS) / N_BUCKETS
            else:
                matrix[r] = (matrix[r] + 0.01) / (row_sum + 0.01 * N_BUCKETS)
        else:
            row_sum = sum(matrix[r])
            if row_sum == 0:
                matrix[r] = [1.0 / N_BUCKETS] * N_BUCKETS
            else:
                total = row_sum + 0.01 * N_BUCKETS
...
def simulate_paths(matrix, start_bucket: int, steps: int) -> float:
    """يحاكي N_SIMULATIONS مسار ويحسب احتمال الانتهاء في [0.9, 1.0]."""
    if steps <= 0:
        steps = 1
    yes_bucket = N_BUCKETS - 1  # bucket [0.9, 1.0]

    if HAS_NUMPY:
        # بناء CDF لكل صف للسحب السريع
        ...
        for _ in range(steps):
            ...
            for i in range(N_SIMULATIONS):
                buckets[i] = np.searchsorted(cdf[buckets[i]], rands[i])
                if buckets[i] >= N_BUCKETS:
                    buckets[i] = N_BUCKETS - 1
        counts = int(np.sum(buckets == yes_bucket))
    else:
        # بناء CDF يدوي
        cdf = []
        for r in range(N_BUCKETS):
        ...
        for _ in range(N_SIMULATIONS):
            ...
            for __ in range(steps):
                ...
                for c in range(N_BUCKETS):
                    if rand_val <= cdf[bucket][c]:
                        bucket = c
                        break
            if bucket == yes_bucket:
                counts += 1

    return counts / N_SIMULATIONS

# ═══════════════════════════════════════════════
# تحليل ماركوف لسوق واحد
# ═══════════════════════════════════════════════
def analyze_market(mkt: dict) -> dict | None:
    """يحلل سوق واحد — يبني المصفوفة ويحاكي ويحسب الفجوة."""
    token_id = mkt["yes_token"]
    prices = fetch_price_history(token_id)
    synthetic = False
    if len(prices) < 24:
        prices = synthesize_history(mkt["yes_price"])
        synthetic = True

    matrix = build_transition_matrix(prices)
    current_bucket = price_to_bucket(mkt["yes_price"])
    steps_to_expiry = max(1, int(mkt["hours_left"]))

    p_yes = simulate_paths(matrix, current_bucket, steps_to_expiry)
    market_yes = mkt["yes_price"]
    edge_yes = p_yes - market_yes
    edge_no = (1 - p_yes) - mkt["no_price"]

    signal = None
    if edge_yes >= MIN_EDGE:
        signal = {"side": "YES", "edge": edge_yes, "p_calc": p_yes}
    elif edge_no >= MIN_EDGE:
        signal = {"side": "NO", "edge": edge_no, "p_calc": 1 - p_yes}
    ...
    return {
        ...
        "p_calculated": round(signal["p_calc"], 4),
        "market_price": round(market_yes if signal["side"] == "YES" else mkt["no_price"], 4),
        "yes_price": market_yes,
        "no_price": mkt["no_price"],
        "hours_left": mkt["hours_left"],
        "volume": mkt["volume"],
...
def check_positions(st: dict, mode: str) -> dict:
    ...
    for pos in st["positions"]:
        token_id = pos["yes_token"] if pos["side"] == "YES" else pos["no_token"]
        ...
        if reason:
            ...
            msg = (f"<b>EXIT</b> | {pos['question'][:40]}\n"
                   f"سبب: {reason} | PnL: ${pnl_usd:+.2f}\n"
                   f"دخول: {entry:.3f} خروج: {current:.3f}")
            log(f"EXIT: {reason} | {pos['question'][:35]} | ${pnl_usd:+.2f}")
            tg_send(msg)
            closed.append(pos)
    for c in closed:
        st["positions"].remove(c)
    return st
...
def show_status(st: dict):
    ...
    print("=" * 55)
    print(f"  اليوم: {st['day']}")
    print(f"  صفقات: {st['daily_trades']}/{MAX_TRADES_DAY}")
    print(f"  PnL اليوم: ${st['daily_pnl']:.2f}")
    print(f"  مراكز: {len(st['positions'])}/{MAX_POSITIONS}")
...
def run_loop(mode: str):
    ...
    tg_send(f"<b>Markov Bot</b> شغّال | وضع: {mode}\n"
            f"sims={N_SIMULATIONS:,} | edge>={MIN_EDGE:.0%}")
    ...
    while True:
        ...
        if st["daily_trades"] >= MAX_TRADES_DAY:
            log("الحد اليومي للصفقات — انتظار")
            time.sleep(SCAN_INTERVAL)
            continue
        if len(st["positions"]) >= MAX_POSITIONS:
            log("الحد الأقصى للمراكز — انتظار")
            time.sleep(SCAN_INTERVAL)
            continue

        log(f"[{now_str}] فحص #{cycle} — جلب الأسواق...")
        markets = fetch_active_markets()
        if not markets:
            log("لا توجد أسواق مؤهلة")
            time.sleep(SCAN_INTERVAL)
            continue
        ...
        signals = []
        for mkt in markets:
            # تخطي أسواق مفتوحة مسبقا
            existing = [p for p in st["positions"] if p["market_id"] == mkt["market_id"]]
            if existing:
                continue
            try:
                ...
                log(f"خطأ تحليل {mkt['question'][:30]}: {e}", "WARNING")
            time.sleep(0.3)

        if not signals:
            log(f"لا إشارات — {len(markets)} سوق بدون فجوة كافية")
            time.sleep(SCAN_INTERVAL)
            continue
        ...
        for sig in signals:
            if st["daily_trades"] >= MAX_TRADES_DAY or len(st["positions"]) >= MAX_POSITIONS:
                break

            q_short = sig["question"][:40]
            hist_tag = "حقيقي" if not sig["synthetic"] else "اصطناعي"
            signal_msg = (
                f"<b>SIGNAL</b> | {sig['side']} {q_short}\n"
                f"P(محسوب)={sig['p_calculated']:.1%} vs سوق={sig['market_price']:.1%}\n"
                f"فجوة: {sig['edge']:.1%} | بيانات: {sig['data_points']} ({hist_tag})\n"
                f"حجم: ${sig['volume']:,.0f} | متبقي: {sig['hours_left']:.0f}h")
            log(f"SIGNAL: {sig['side']} {q_short} | edge={sig['edge']:.1%} "
                f"| calc={sig['p_calculated']:.2f} vs mkt={sig['market_price']:.2f}")
            tg_send(signal_msg)
            ...
            if ok:
                pos = {
                    ...
                    "p_calculated": sig["p_calculated"],
                    "edge": sig["edge"],
                    "size_usd": TRADE_SIZE,
                    "yes_token": sig["yes_token"],
                    "no_token": sig["no_token"],
                    "entry_time": datetime.now(timezone.utc).isoformat(),
                }
                st["positions"].append(pos)
                st["daily_trades"] += 1
                st["trade_log"].append({**pos, "result": result[:80],
                                        "mode": mode, "cycle": cycle})
                trade_msg = (f"<b>TRADE</b> | {sig['side']} {q_short}\n"
                             f"${TRADE_SIZE} @ {sig['market_price']:.3f}\n{result[:60]}")
                log(f"TRADE: {sig['side']} {q_short} | ${TRADE_SIZE} | {result[:50]}")
                tg_send(trade_msg)
            else:
                log(f"فشل التنفيذ: {result[:80]}", "WARNING")
...
# ═══════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Markov Chain Forecaster — Polymarket")
    ...
    if args.live:
        if not PK:
            print("PK مفقود في .env3 — لا يمكن التداول الحقيقي")
            return
        mode = "live"
    else:
        mode = "paper"
...
{'='*55}
  ...
  الوضع: {'تجريبي' if mode == 'paper' else 'حقيقي'}
  Buckets: {N_BUCKETS} | Simulations: {N_SIMULATIONS:,}
  Lookback: {LOOKBACK_DAYS} days | Min Edge: {MIN_EDGE:.0%}
  numpy: {'YES' if HAS_NUMPY else 'NO (pure Python fallback)'}
{'='*55}
""")
    try:
        run_loop(mode)
